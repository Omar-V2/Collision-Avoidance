...
class Robot:
	def __init__(self, x, y, size, field_of_view, num_sensors, manager, set_weights, own_weights=False):
		...
		if own_weights:
			self.brain = Neural.NeuralNetwork(inputs=num_sensors, hidden_layers=3, hidden_neurons=16, outputs=2,
			                                  given_weights=set_weights)
		else:
			self.brain = Neural.NeuralNetwork(inputs=num_sensors, hidden_layers=3, hidden_neurons=16, outputs=2,
			                                  given_weights=self.create_weights(num_sensors, 16, 3,
			                                                                    2))  # this will be the neural network which makes the decision based on sensor inputs
	...
	def move(self):
		if self.alive:
			...
			self.x += 15 * self.brain.forward([(sensor.reading / self.max_range) for sensor in self.sensors])[0] * m.cos(
				m.radians(self.robot_angle))
			self.y += 15 * self.brain.forward([(sensor.reading / self.max_range) for sensor in self.sensors])[0] * m.sin(
				m.radians(self.robot_angle))
		# 	self.robot_angle += self.brain.forward([(sensor.reading/self.max_range) for sensor in self.sensors])[1] # this changes between -1 and 1 degrees per decision
	...
	def update(self):
		if self.alive:
			...
			for sensor in self.sensors:
				...
				# sensor.detect(self.manager, self)
				# sensor.collide(self, self.manager)
				# sensor.detect_static()
				# sensor.detect_static2()
		if time.time() - self.time_alive > 4:  # add condition to check if fitness isnt changing much not just time for killing
			self.alive = False
	...
	def collide(self):
		...
		# 				self.alive = False
		if self.x <= 10 or self.y <= 10 or self.y >= height - 20 or self.x >= width - 20:  # or self.x >= 800 :
			self.alive = False
	...
	def evaluate_fitness(self):
		if self.alive:
			...
			closest = total
			# finish_line = robot_pos.get_distance()
			# completion_time = self.sensors[0].completion_time
			# pygame.draw.line(screen, (0, 0, 255), (robot_pos), (800, self.y))
			finish_line = robot_pos.get_distance(Vec2d(800, self.y))
			# print(finish_line)
			...
			if self.hitTarget:
				...
				targetFactor = 0
			try:
				# self.fitness = (1 / finish_line)
				# self.fitness = (1 / self.best)
				# self.fitness = 1/(distance*m.cos(m.radians(angle)))
				self.fitness = (1 / distance) + (0.5 * (1 / self.best)) + (0.3 * (1 / self.time_alive)) + (0.5 * targetFactor)
...
class Sensor:
	...
	def update_sensor_pos(self, robot):
		...
		self.x0 = robot.x + self.radius * m.cos(
			m.radians(self.angle + robot.robot_angle))  # do robot_angle + self.angle here
	...
	def detect(self, manager, robot):
		...
		for pedestrian in manager.start_pedestrians:
			...
			q = Vec2d(h, k)
			r = pedestrian.size
			distance_to_ped = robot_pos.get_distance(q)
			if distance_to_ped <= robot.size + self.max_range + r:
	...
	def detectBoundary(self):
		...
		for i in range(len(boundary_coords)):
			hit_boundary = calculateIntersectPoint((self.x0, self.y0), (self.x1, self.y1), boundary_coords[i][0],
			                                       boundary_coords[i][1])
